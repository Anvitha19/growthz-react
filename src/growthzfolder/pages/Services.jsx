...
const servicesData = [
    {
      title: '3X EFFECTIVE SEO WITH STRATEGIC LINK BUILDING LINK BUILDING',
  ...
  return (
    ...
    <div className='font-work md:py-10 w-full max-[876px]:py-8 bg-[#6E18F3]'>
        <div className='relative py-[100px] mx-auto flex-auto items-center justify-center w-[90%]'>
            ...
            <div className='pt-12'>
            <button className='font-work bg-[#f3be18] text-sm font-black text-black py-2.5 px-8 rounded-2xl hover:bg-black duration:300'>LET'S TALK</button>
            </div>
        </div>
    </div>
    {/* Banner end */}
    <div className='font-work w-full md:pt-16'>
  ` <div className='max-w-[1240px] mx-auto grid md:grid-cols-2 gap-8 justify-between'>
    {servicesData.map((section, index) => (
      <div key={index} className='flex flex-col justify-center py-10'>
        <h1 className='md:text-[57px] md:leading-[53px] sm:text-2xl text-xl font-black py-2'>{section.title}</h1>
...
<div className='font-work w-full'>
    <div className='mx-auto w-[90%]  max-[876px]:w-[80%]'>
        ...
        <div className='mx-auto grid md:grid-cols-3 flex justify-center w-[90%]'>
        {listData.map((list, index) => (
        <ul key={index} className=''>
        <li className='mb-14 list-disc md:text-2xl font-bold max-[876px]:text-[16px]'>{list.name}</li>
        </ul>
        ))}
        </div>
    </div>
